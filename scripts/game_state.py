...
class Player:
    ...
    def get_account_status(self, current_price):
        ...
        if self.stock >= 0: return "✅ 正常"
        short_val, frozen, avail, risk = self.get_margin_info(current_price)
        if risk < 1.15: return "🆘 濒临强平"
        elif risk < 1.35: return "⚠️ 保证金告急"
        elif avail < 5000: return "🔒 资产冻结"
        else: return "📉 做空持仓中"
...
class GameState:
    ...
    def reset(self):
        ...
        # === 新增 K线数据结构 ===
        # 格式: [{'time': 0, 'open': 100, 'high': 102, 'low': 99, 'close': 101, 'volume': 5000}]
        self.kline_data = [] 
        self.current_open = 100.0 # 当前小时开盘价
        self.current_volume = 0   # 当前小时成交量 (股数)
    ...
    def next_hour(self):
        ...
        # 3. 生成本小时的 High/Low (模拟影线)
        # 逻辑：最高价肯定比Open/Close都高一点，最低价低一点
        # 波动幅度基于当天的波动率
        ...
        self.kline_data.append({
            'time': self.game_clock, # 第0小时, 第1小时...
            'open': hour_open,
            'high': hour_high,
            'low': hour_low,
            'close': hour_close,
        ...
        # 为下一小时做准备
        self.current_open = self.current_price # 下一小时开盘价 = 本小时收盘价
        self.current_volume = 0                # 重置成交量
        self.current_momentum = 0.0 
